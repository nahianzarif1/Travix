...
Route::middleware('auth')->group(function () {
    ...
    // Payment routes (bKash only via SSLCommerz)

Route::middleware('auth')->group(function () {

    // Show checkout page (only one option — bKash)
    Route::get('/payment', [PaymentController::class, 'checkout'])->name('payment');

    // Initiate bKash payment (redirects to SSLCommerz page)
    Route::post('/payment/initiate', [PaymentController::class, 'initiate'])->name('payment.initiate');

    // SSLCommerz return URLs
...
Route::middleware('auth')->group(function () {
    ...
    Route::get('/flights/search', function (Request $request) {
        $query = Flight::query();
        if ($request->filled('from')) $query->where('from_city', $request->from);
        if ($request->filled('to')) $query->where('to_city', $request->to);

        $flights = $query->get();
        return redirect()->route('home')->with('search_results', $flights)->withFragment('flights');
    })->name('flights.search');
    ...
    Route::get('/hotels/search', function (Request $request) {
        ...
        $hotels = $query->get();
        return redirect()->route('home')->with('search_results', $hotels)->withFragment('hotels');
    })->name('hotels.search');
